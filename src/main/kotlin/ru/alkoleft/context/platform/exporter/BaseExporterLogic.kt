...
class BaseExporterLogic : ExporterLogic {
    ...
    override fun extractProperties(context: PlatformGlobalContext): Stream<PropertyDefinition> {
        ...
        return Optional.ofNullable(context.properties())
            ...
            .orElse(Stream.empty())
            .map { property ->
                val type = Optional.ofNullable(property.types())
    ...
    override fun extractMethods(context: PlatformGlobalContext): Stream<MethodDefinition> {
        ...
        return Optional.ofNullable(context.methods()).stream()
            .flatMap { it.stream() }
            .map { method ->
                val signatures = Optional.ofNullable(method.signatures())
                    .map { sigs ->
                        sigs.map { toSignature(it) }
                    }
                    .orElse(emptyList())

                val returnValue = if (method.hasReturnValue() &&
    ...
    private fun toSignature(sig: ContextMethodSignature): Signature {
        val sigDescription = if (PRIMARY_SIGNATURE_NAME == sig.name().name) {
            sig.description()
        } else {
            "${sig.name().name}. ${sig.description()}"
        }

        val paramsList = Optional.ofNullable(sig.parameters())
    ...
    override fun extractTypes(contexts: List<Context>): Stream<PlatformTypeDefinition> {
        return Optional.ofNullable(contexts).stream()
            .flatMap { it.stream() }
